...
class AliceVisionRecipe(ConanFile):
    ...
    default_options = {
        ...
        # opencv/4.11.0: Invalid: Several opencv options are disabled but are required by modules you have explicitly requested:
        #   - ximgproc: direct dependency of optflow
        ...
        # Also disable opencolorio transitive dependency to workaround https://github.com/conan-io/conan-center-index/issues/25765 for static build
    ...
    def requirements(self):
        ...
        # Doesn't work with a more recent release
        # src/software/pipeline/CMakeFiles/aliceVision_cameraInit_exe.dir/main_cameraInit.cpp.o:main_cameraInit.cpp:vtable for boost::program_options::validation_error:(.data.rel.ro._ZTVN5boost15program_options16validation_errorE+0x20)<C2><A0>: erreur<C2><A0>: r<C3><A9>f<C3><A9>rence <C3><A0> <C2><AB><C2><A0>boost::program_options::error_with_option_name::what() const<C2><A0><C2><BB> non d<C3><A9>finie
...
# /root/.conan2/p/nanofdf3cf2c8ea57d/p/include/nanoflann.hpp:1697:41: error: <E2><80><98>class aliceVision::fuseCut::SmallerPixSizeInRadius<double, long unsigned int><E2><80><99> has no member named <E2><80><98>sort<E2><80><99>
...
#         # TODO: add geogram to conan-center
        if self.options.with_sfm:
          ...
          self.requires("coin-utils/2.11.9", force=True)
          self.requires("coin-clp/[>=1.17.7 <2]")
          #self.requires("coin-cbc/[>=2.10.5 <3]")
         ...
         # See https://github.com/conan-io/conan-center-index/pull/23536
          ...
          self.requires("geogram/[>=1.9.0 <2]")
          self.requires("openmesh/11.0")
          self.requires("expat/2.7.1")
        if self.options.with_sfm or self.options.with_mvs:
          self.requires("assimp/5.4.3")
          self.requires("minizip-ng/4.0.10", force=True)


        self.requires("openimageio/2.5.18.0")
        # self.requires("openimageio/2.5.18.0", options={"with_ffmpeg":False, "with_opencolorio": False})
#         self.requires("coin-osi/0.108.7", force=True)

#         self.requires("hdf5/1.14.6", force=True)
#         self.requires("cpuinfo/cci.20250110", force=True)
...
#         # Static build of onetbb not recommeneded https://github.com/uxlfoundation/oneTBB/issues/297
...
#         # # Requires python 3.8+ to be built as shared.
        if self.options.with_onnx:
            # Failed:
#             /home/esteban/.conan2/p/b/onnxr732b684e65201/b/src/onnxruntime/core/providers/cpu/math/element_wise_ops.cc:967:85: error: type/value mismatch at argument 1 in template parameter list for <E2><80><98>template<class OtherDerived> const Eigen::CwiseBinaryOp<Eigen::internal::scalar_max_op<typename Eigen::internal::traits<T>::Scalar, typename Eigen::internal::traits<OtherDerived>::Scalar>, const Derived, const OtherDerived> Eigen::ArrayBase<Derived>::max(const Eigen::ArrayBase<OtherDerived>&) const [with Derived = Eigen::ArrayWrapper<Eigen::Map<const Eigen::Matrix<long unsigned int, -1, 1, 0, -1, 1>, 0, Eigen::Stride<0, 0> > >]<E2><80><99>
#   967 |               per_iter_bh.EigenInput0<T>().array().template max<Eigen::PropagateNaN>(
#       |               ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~^
#   968 |                   per_iter_bh.EigenInput1<T>().array());
            # self.requires("onnxruntime/1.21.1")
...
#         # self.requires("openusd/24.08")
        if self.options.with_lidar:
          self.requires("libe57format/3.2.0")
...
#         # self.requires("pcl/1.15.0", options={"with_libusb": False})
        
        #self.requires("openblas/0.3.27", force=True)
        # # MeshSDFilter?
        
        if self.options.with_opencv:
            self.requires("opencv/4.12.0")
            # self.requires("opencv/4.12.0", options={"with_wayland": False})
   #   )
...
#         # # UncertaintyTE?
    ...
    def generate(self):
        ...
        tc.cache_variables["ALICEVISION_BUILD_DOC"] = False
        tc.cache_variables["ALICEVISION_BUILD_SFM"] = self.options.with_sfm
        self.output.info(f"with_mvs: {self.options.with_mvs}")
        tc.cache_variables["ALICEVISION_BUILD_MVS"] = self.options.with_mvs
        tc.cache_variables["ALICEVISION_BUILD_HDR"] = self.options.with_hdr
        tc.cache_variables["ALICEVISION_BUILD_SEGMENTATION"] = self.options.with_segmentation
        tc.cache_variables["ALICEVISION_BUILD_STEREOPHOTOMETRY"] = self.options.with_stereophometry
        tc.cache_variables["ALICEVISION_BUILD_PANORAMA"] = self.options.with_panorama
        tc.cache_variables["ALICEVISION_BUILD_SOFTWARE"] = self.options.with_cli
        
        tc.cache_variables["ALICEVISION_USE_OPENMP"] = "ON" if self.options.with_openmp else "OFF"
        tc.cache_variables["ALICEVISION_USE_CCTAG"] = "ON" if self.options.with_cctag else "OFF"
        tc.cache_variables["ALICEVISION_USE_APRILTAG"] = "ON" if self.options.with_apriltag else "OFF"
        tc.cache_variables["ALICEVISION_USE_POPSIFT"] = "ON" if self.options.with_popsift else "OFF"
        tc.cache_variables["ALICEVISION_USE_OPENGV"] = "ON" if self.options.with_opengv else "OFF"
        tc.cache_variables["ALICEVISION_USE_ALEMBIC"] = "ON" if self.options.with_alembic else "OFF"
        tc.cache_variables["ALICEVISION_USE_UNCERTAINTYTE"] = "ON" if self.options.with_uncertaintype else "OFF"
        tc.cache_variables["ALICEVISION_USE_ONNX"] = "ON" if self.options.with_onnx else "OFF"
        tc.cache_variables["ALICEVISION_USE_ONNX_GPU"] = "ON" if self.options.with_onnx_gpu else "OFF"
        tc.cache_variables["ALICEVISION_USE_OPENCV"] = 'ON' if self.options.with_opencv else "OFF"
        tc.cache_variables["ALICEVISION_USE_OPENCV_CONTRIB"] = 'ON' if self.options.with_opencv else "OFF"
        tc.cache_variables["ALICEVISION_USE_CUDA"] = "ON" if self.options.with_cuda else "OFF"
        tc.cache_variables["ALICEVISION_USE_MESHSDFILTER"] = False
        tc.cache_variables["ALICEVISION_BUILD_LIDAR"] = "ON" if self.options.with_lidar else "OFF"
        # tc.cache_variables["AV_BUILD_LAPACK"] = False
        ...
        # tc.cache_variables["LEMON_LIBRARY"] = self.dependencies['lemon'].cpp_info.libdirs[0]
        
        tc.generate()

        deps = CMakeDeps(self)
        # deps.set_property("coin-osi", "cmake_target_name", "")
        deps.set_property("nanoflann", "cmake_target_name", "nanoflann")
        # See https://github.com/conan-io/conan-center-index/issues/25765
        # deps.set_property("minizip", "cmake_file_name", "oldminizip")
        # deps.set_property("minizip", "cmake_target_name", "MINIZIP::minizip")
        # deps.set_property("minizip", "cmake_target_name", "MINIZIP")
        
        deps.set_property("onnxruntime", "cmake_file_name", "ONNXRuntime")
        deps.set_property("onnxruntime", "cmake_target_name", "ONNXRuntime::ONNXRuntime")
