...
Create a conan remote repository named conancenter to https://center2.conan.io/
...
<!-- See https://github.com/conan-io/conan-center-index/blob/master/docs/supported_platforms_and_configurations.md#linux -->
conan profile detect 
set Debug
set cptstd to 17 instead of gnu14
set compiler version to 11 instead of 12

https://www.baeldung.com/linux/shared-library-exported-functions
  nm -D --demangle

Or using https://github.com/valgur/conan-center-index/ fork
...
sudo docker run --rm -ti  debian:bookworm
apt-get -y update
apt-get install -y crossbuild-essential-arm64 pipx cmake ccache ninja-build ssh git vim pkg-config wget software-properties-common 

sudo docker run -it conanio/gcc11-ubuntu18.04
pip install --upgrade conan
git clone https://github.com/EstebanDugueperoux2/AliceVision.git
cd AliceVision
git switch feature/AddConanSupport
conan install . -s build_type=Debug --profile:build .conan/profiles/build_profile --profile:host .conan/profiles/build_profile --build missing -c tools.system.package_manager:mode=install -c tools.system.package_manager:sudo=True &> install_static.log 
